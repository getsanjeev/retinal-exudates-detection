...
if __name__ == "__main__":
	...
	Y_train = dataset_train[:,3]

	print(dataset_train[50:55,3])

	X_test = dataset_test[:,0:2]
	Y_test = dataset_test[:,3]
	

	print("DECISION TREE")
	clf_decision_tree = AdaBoostClassifier(DecisionTreeClassifier(max_depth = 5),algorithm="SAMME",n_estimators=100)
	...
	Y_predicted = clf_adaboost.predict(X_test)
	print (Y_predicted)
	print("accuracy")
	print(accuracy_score(Y_test, Y_predicted))
	print("confusion matrix")
	...
	clf_sdg = SGDClassifier(n_iter=100, alpha=0.01)
	clf_sdg.fit(X_train, Y_train)
	Y_predicted = clf_sdg.predict(X_test)
	print (Y_predicted)
	print("accuracy")
	print(accuracy_score(Y_test, Y_predicted))
	print("confusion matrix")
